...
from PyQt5.QtWidgets import (
    ...
    QLineEdit, QFormLayout, QScrollArea, QSizePolicy, QScrollBar
)
...
def _load_core():
    ...
    for n in names:
        ...
        if os.path.exists(p):
            try:
                lib = ctypes.CDLL(p)  # ← BURASI DÜZELTİLDİ (lib tanımlandı)
                _setup(lib)
                _core_path = p
...
MODE_IMAGE   = 3
MODE_PDF     = 4
MODE_VID_MIX = 5 # Yeni Mod: Videoya Harici Ses Ekleme
...
def mix_video_and_audio(vid_path, aud_path, out_path, do_inv, do_gray, is_preview, progress_cb):
    ...
    try:
        ...
        if a_dur > v_dur:
            ...
            # DÜZELTME: tpad'in 'stop_mode=black' yerine FFmpeg 6.1 ile %100 uyumlu sayısal 'stop_mode=0' (Varsayılan siyah dolgu) kullanılır.
        ...
        cmd.extend([
            ...
            '-pix_fmt', 'yuv420p',
            '-c:a', 'aac', '-b:a', '192k',
            '-shortest' # Tüm girdilerden en kısası kadar çıktı boyutlandırılır (loop girdisini otomatik frenler)
        ])
...
class UniversalConverter(QWidget):
    ...
    def _start_preview_pipeline(self):
        ...
        if self.conv_thread and self.conv_thread.isRunning():
            QMessageBox.warning(self, "Uyarı", "Şu an başka bir dönüştürme işlemi devam ediyor.")
            return
        ...
        if self._cached_preview_path and os.path.exists(self._cached_preview_path) and self._cached_preview_settings == current_hash:
            self._cached_preview_path = self._cached_preview_path
    ...
    def _start_video_preview_playback(self, video_path):
        ...
        try:
            self._video_preview_process = subprocess.Popen(
                [FFPLAY_PATH, '-autoexit', '-window_title', 'Önizleme (5 sn)', video_path],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            self._is_playing = True
            self.btn_play_preview.setText("Pauze")
            ...
            QMessageBox.warning(self, "Uyarı", f"ffplay bulunamadı: {FFPLAY_PATH}")
    ...
    def _select_vid_audio(self):
        ...
        if p and os.path.exists(p):
            ...
            self.c_vid_rm_aud.setCurrentIndex(0) # Yeni ses seçildiğinde tam silme iptal olur
            self._invalidate_preview_cache()
