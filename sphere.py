...
class SphereWindow(QWidget):
    ...
    def reset_environment(self):
        ...
        def do_reset():
            ...
            if self.core_window_ref:
                ...
                self.core_window_ref.save_custom_editors()
    ...
    def handleTerminalCommand(self, command):
        ...
        if cmd.endswith(" Kavram") and len(cmd) > 7:
            parts = cmd.rsplit(" ", 1)
            if len(parts) == 2 and parts[1] == "Kavram":
                ...
                target_dir = os.path.join(self.custom_editors_dir, editor_name)
                os.makedirs(target_dir, exist_ok=True)
                base_name = os.path.basename(source_exec)
                target_exec = os.path.join(target_dir, base_name)
                try:
                ...
                if self.core_window_ref and hasattr(self.core_window_ref, 'add_custom_editor'):
                    success = self.core_window_ref.add_custom_editor(editor_name, target_exec)
                    if success:
                        self.show_info_message(f"'{editor_name}' editörü başarıyla sisteme eklendi.")
                    else:
                        try:
                            os.remove(target_exec)
                            os.rmdir(target_dir)
                        except:
                            pass
                else:
