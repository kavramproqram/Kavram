...
try:
    from kare import DraggableBox, DraggableProxyWidget, format_filename_for_display
except ImportError:
    ...
    def format_filename_for_display(file_name, font_metrics=None, available_width=180):
        # Fallback: kare.py yüklenemiyorsa gerçek dosya adını değiştirmeden göster.
        return file_name or ""
    ...
    def addDraggableBox(self, pos=None, editor_name=None, file_path=None, box_data=None):
        ...
        if file_path and os.path.exists(file_path):
            ...
            display_name = format_filename_for_display(
                file_name,
                box.file_list_widget.fontMetrics(),
                box.file_list_widget.viewport().width() - 12
            )
            item = QListWidgetItem(display_name)
            item.setData(Qt.UserRole, file_path)
            line_count = max(1, display_name.count("\n") + 1)
            item.setSizeHint(QSize(180, min(120, line_count * box.file_list_widget.fontMetrics().height() + 8)))
...
class SphereWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.file_button.clicked.connect(lambda: self.open_file_dialog_for_new_box())
        # Sağ tık için: menü açmak yerine doğrudan "Tüm Dosyalar" ile pencere aç
        self.file_button.setContextMenuPolicy(Qt.CustomContextMenu)
        self.file_button.customContextMenuRequested.connect(lambda pos: self.open_file_dialog_for_new_box(all_files=True))
    ...
    # ===================== DÜZELTİLMİŞ reset_environment METODU =====================
    def reset_environment(self):
        ...
        folders_to_clean = [
            ...
            os.path.join(os.path.expanduser('~'), 'Kavram', 'Proqramlar')
        ]
        ...
        auxiliary_programs = self.core_window_ref.auxiliary_programs if self.core_window_ref and hasattr(self.core_window_ref, 'auxiliary_programs') else []
        auxiliary_names = [e['name'] for e in auxiliary_programs]
        ...
        text_edit.append("\nSilinecek özel editörler (üst):")
        if custom_editor_names:
            text_edit.append("\n".join([f"• {name}" for name in custom_editor_names]))
        else:
            text_edit.append("(Hiçbir özel editör yok)")
        text_edit.append("\nSilinecek uydu programlar (alt):")
        if auxiliary_names:
            text_edit.append("\n".join([f"• {name}" for name in auxiliary_names]))
        else:
            text_edit.append("(Hiçbir uydu program yok)")
        ...
        def do_reset():
            ...
            # Uydu program temizliği (alt) - DÜZELTİLDİ
            if self.core_window_ref and hasattr(self.core_window_ref, 'get_auxiliary_programs_json_path'):
                auxiliary_programs_path = self.core_window_ref.get_auxiliary_programs_json_path()
                if os.path.exists(auxiliary_programs_path):
                    try:
                        os.remove(auxiliary_programs_path)
                    except Exception as e:
                        print(f"auxiliary_programs.json silinemedi: {e}")
                # Önce kutu temizliği
                for aux_name in auxiliary_names:
                    self.core_window_ref.cleanup_boxes_for_auxiliary_program(aux_name)
                # Ardından listeden ve haritalardan sil
                self.core_window_ref.auxiliary_programs.clear()
                for aux_name in auxiliary_names:
                    if aux_name in self.core_window_ref.editor_map:
                        del self.core_window_ref.editor_map[aux_name]
                    if aux_name in self.core_window_ref.editors_order:
                        self.core_window_ref.editors_order.remove(aux_name)
                # Yeniden sırala
                self.core_window_ref.rebuild_editors_order()
                self.core_window_ref.save_auxiliary_programs_json()
    ...
    def handleTerminalCommand(self, command):
        ...
        # ---------- ÜST ÖZEL DÜZENLEYİCİ EKLEME (k ...) ----------
        if cmd.startswith("k "):
            parts = raw_cmd.split(maxsplit=2)
            if len(parts) < 3:
                self.show_error_message(
                    "Kullanım: k <program_ismi> <çalıştırılabilir_dosya_yolu>\n"
                    "Örnek: k blender /usr/bin/blender"
                )
                return

            program_name = parts[1].strip()
            executable_path = parts[2].strip()

            if not program_name:
                self.show_error_message("Program adı boş olamaz.")
                return
            if not executable_path:
                self.show_error_message("Çalıştırılabilir dosya yolu boş olamaz.")
                return
            if not os.path.exists(executable_path):
                self.show_error_message(f"Dosya bulunamadı: {executable_path}")
                return
            if not os.access(executable_path, os.X_OK):
                self.show_error_message(f"Dosya çalıştırılabilir değil: {executable_path}")
                return

            if not self.core_window_ref or not hasattr(self.core_window_ref, 'add_custom_editor'):
                self.show_error_message("CoreWindow üst özel düzenleyici desteğini sağlamıyor.")
                return

            if self.core_window_ref.add_custom_editor(program_name, executable_path):
                self.show_info_message(f"Üst özel düzenleyici eklendi: {program_name} -> {executable_path}")
            return

        # ---------- UYDU PROGRAM EKLEME (program <isim> <yol> veya <isim> <yol>) ----------
        # Normal komut: program veya iki argüman
        if cmd.startswith("program "):
            parts = raw_cmd.split(maxsplit=2)
            if len(parts) < 3:
                self.show_error_message(
                    "Kullanım: program <program_ismi> <çalıştırılabilir_dosya_yolu>\n"
                    "Örnek: program blender /usr/bin/blender"
                )
                return
            ...
            # İki argümanlı komut (program olmadan)
            parts = raw_cmd.split(maxsplit=2)
            if len(parts) < 2:
                self.show_error_message(
                    "Geçersiz komut. Kullanılabilir komutlar: \n"
                    "reset, ap, ap <resim_yolu>, \n"
                    "k <isim> <çalıştırılabilir_yol>  (üst özel)\n"
                    "program <isim> <çalıştırılabilir_yol> veya <isim> <çalıştırılabilir_yol> (alt yardımcı)"
                )
                return
            program_name = parts[0].strip()
            executable_path = parts[1].strip()
            if len(parts) == 3:  # yolda boşluk varsa
                executable_path = parts[1] + " " + parts[2]

        if not program_name:
            self.show_error_message("Uydu program adı boş olamaz.")
            return
        if not executable_path:
            self.show_error_message("Çalıştırılabilir dosya yolu boş olamaz.")
            return
        ...
        if not self.core_window_ref or not hasattr(self.core_window_ref, 'add_auxiliary_program'):
            self.show_error_message("CoreWindow uydu program desteğini sağlamıyor.")
            return

        if self.core_window_ref.add_auxiliary_program(program_name, executable_path):
            self.show_info_message(f"Uydu program eklendi: {program_name} -> {executable_path}")
        else:
            self.show_error_message("Uydu program eklenirken hata oluştu (sınır aşılmış olabilir).")
    ...
    def export_data(self, compression="xz"):
        """10 GB gibi devasa projeleri dahi düşük RAM ile dışa aktaran tam akışlı (stream) export."""
        if not CRYPTO_AVAILABLE:
        ...
        try:
            ...
            progress_dialog.update_progress(30, f"Arşiv doğrudan akış ile oluşturuluyor ({compression.upper()})...")
            ...
            with tarfile.open(tmp_tar_path, tar_mode) as tar:
                ...
                for idx, (final_name, origin_path) in enumerate(files_to_export.items(), 1):
                    tar.add(origin_path, arcname=final_name, recursive=False)
                    p_val = 30 + int((idx / max(1, total_files)) * 30)
                    progress_dialog.update_progress(p_val, f"Arşive akıtılıyor: {final_name}")

            progress_dialog.update_progress(60, "Veri akış halinde şifreleniyor...")

            def enc_progress(proc, total):
                p_val = 60 + int((proc / max(1, total)) * 35)
                progress_dialog.update_progress(p_val, f"Şifreleniyor... (%{int((proc / max(1, total)) * 100)})")

            encrypt_file_stream_gcm(tmp_tar_path, file_name, password, progress_callback=enc_progress)

            progress_dialog.update_progress(100, "Tamamlandı!")
            time.sleep(0.5)
            fmt_info = "Hızlı (gz)" if compression == "gz" else "Yüksek Sıkıştırma (xz)"
            self.show_info_message(f"Proje {fmt_info} formatında \nbaşarıyla '{os.path.basename(file_name)}' \nolarak dışa aktarıldı.")
    ...
    def open_file_dialog_for_new_box(self, all_files=False):
        """File butonu ile dosya seçme diyalogu açar. .blend uzantısı için Blender editörünü atar."""
        ...
        if all_files:
            ...
            file_filter = (
                "Kitap Files (*.kitap);;"
                "All Supported Files (*.kitap *.txr *.png *.pnf *.jpg *.jpeg *.bmp *.gif *.ai *.sound *.wav *.aiff *.flac *.ogg *.mp3 *.media *.mp4 *.avi *.mov *.mkv *.webm *.flv *.rec *.aac *.m4a *.copya *.blend);;"
                "Program (*);;"
                "All Files (*)"
            )
        ...
        for path in selected_file_paths:
            ...
            box = self.view.addDraggableBox(editor_name=editor_name, file_path=final_path)
    ...
    def buttonStyle(self):
        return """
            ...
            QComboBox::down-arrow { image: url(data:image/svg+xml;base64,); width: 16px; height: 16px; }
