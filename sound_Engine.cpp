...
struct AudioEngine {
    ...
    // Constructor
    AudioEngine() : current_play_position_ms(0), total_duration_ms(0),
                    is_playing_flag(false), current_speed(1.0f),
                    sample_rate(44100), channels(2), // Varsayılan değerler burada ayarlandı
    ...
    void recalculate_envelope_data() {
        ...
        long frames_per_envelope_point = (sample_rate * ENVELOPE_MS_PER_POINT) / 1000;
        if (frames_per_envelope_point == 0) frames_per_envelope_point = 1;

        for (long i = 0; i < audio_buffer.size() / channels; i += frames_per_envelope_point) {
            float max_amplitude = 0.0f;
            for (long j = 0; j < frames_per_envelope_point; ++j) {
    ...
    int play() {
        ...
        outputParameters.device = Pa_GetDefaultOutputDevice(); // Varsayılan çıkış cihazı
        ...
        if (!deviceInfo) {
            std::cerr << "Error: Could not get device info for default output device." << std::endl;
            return -1;
        }
    ...
    int start_microphone_recording() {
        ...
        inputParameters.device = Pa_GetDefaultInputDevice(); // Varsayılan giriş cihazı
        ...
        if (!deviceInfo) {
            std::cerr << "Error: Could not get device info for default input device." << std::endl;
            return -1;
        }
                                ...
                                void *userData)
{
    ...
    if (start_idx + frames_to_write * engine->channels > engine->audio_buffer.size()) {
        ...
        if (frames_to_write < 0) frames_to_write = 0; // Negatif olmaması için
    }

    if (frames_to_write > 0) {
        for (unsigned long i = 0; i < frames_to_write; ++i) {
            for (int k = 0; k < engine->channels; ++k) {
                long current_audio_buffer_idx = (engine->playback_frame_index + i) * engine->channels + k;
