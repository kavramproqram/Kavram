...
struct AudioEngine {
    ...
    ~AudioEngine() {
        ...
        de_hum_a1.clear(); de_hum_a2.clear();
    ...
    int delete_segments_by_thickness(int start_ms, int end_ms) {
        ...
        long start_frame = static_cast<long>((static_cast<double>(start_ms) / 1000.0) * sample_rate);
        long end_frame = static_cast<long>((static_cast<double>(end_ms) / 1000.0) * sample_rate);
        long total_frames = static_cast<long>(audio_buffer.size() / static_cast<size_t>(channels));

        start_frame = std::max(0L, std::min(start_frame, total_frames));
        end_frame = std::max(0L, std::min(end_frame, total_frames));

        if (start_frame >= end_frame) return -1;

        /*
         * Seçili bölüm "kalınlık" için referans olarak kullanılır.
         *
         * Eski algoritma yalnızca maksimum örneği referans alıyor ve
         * 150 ms'den kısa bölgeleri/padding alanlarını koruyordu. Bu nedenle
         * görsel olarak aynı kalınlıkta olan kısa veya parçalı sessizlikler
         * kaçabiliyordu.
         *
         * Yeni tarama:
         * - RMS + peak değerlerini birlikte referans alır.
         * - 5 ms çözünürlükte tüm parçayı baştan sona tarar.
         * - Çok küçük aralıkları birleştirir, böylece "delikli" sessizlik kalmaz.
         * - Eski 25 ms padding'i korumaz; gerçekten eşleşen alanı siler.
         * - Seçili örneği her durumda silme listesine ekler.
         */
        float reference_rms = 0.0f;
        float reference_peak = 0.0f;
        double reference_sum_sq = 0.0;

        long reference_frame_count = end_frame - start_frame;

        for (long frame = start_frame; frame < end_frame; ++frame) {
            for (int c = 0; c < channels; ++c) {
                float value = audio_buffer[frame * channels + c];
                float abs_value = std::abs(value);
                reference_peak = std::max(reference_peak, abs_value);
                reference_sum_sq += static_cast<double>(value) * static_cast<double>(value);
            }
        }

        double reference_sample_count =
            static_cast<double>(reference_frame_count) * static_cast<double>(channels);

        reference_rms = static_cast<float>(
            std::sqrt(reference_sum_sq / std::max(1.0, reference_sample_count))
        );

        /*
         * Toleranslar kasıtlı olarak tek bir "max * 1.2" eşiğinden daha
         * kontrollüdür. RMS, waveform kalınlığını; peak ise keskin
         * transientleri kontrol eder.
         *
         * Alt sınır kullanılmaz: seçilen sessizlikten daha sessiz alanlar
         * da aynı sınıfa girer ve silinir.
         */
        constexpr float RMS_TOLERANCE = 0.20f;
        constexpr float PEAK_TOLERANCE = 0.20f;

        float rms_threshold = std::max(reference_rms * (1.0f + RMS_TOLERANCE), 0.00005f);
        float peak_threshold = std::max(reference_peak * (1.0f + PEAK_TOLERANCE), 0.0001f);

        /*
         * 5 ms tarama çözünürlüğü: küçük boşlukları da yakalamak için
         * eski 10 ms pencereden daha yoğundur.
         */
        long window_frames = (sample_rate * 5) / 1000;
        if (window_frames < 1) window_frames = 1;

        /*
         * 20 ms minimum süre yalnızca tek-tük transient/sample hatalarının
         * silinmesini engeller. 150 ms sınırı bilinçli olarak kaldırıldı.
         */
        long min_matching_frames = (sample_rate * 20) / 1000;
        if (min_matching_frames < 1) min_matching_frames = 1;

        /*
         * Arada 10 ms'ye kadar eşleşmeyen küçük bir parça varsa bunu aynı
         * sessizlik bölgesinin devamı kabul ediyoruz. Böylece tarama,
         * geride küçük ses adacıkları bırakmaz.
         */
        long max_bridge_frames = (sample_rate * 10) / 1000;
        if (max_bridge_frames < 1) max_bridge_frames = 1;

        std::vector<std::pair<long, long>> segments_to_delete;
        segments_to_delete.reserve(64);

        long current_match_start = -1;
        long last_match_end = -1;
        long bridge_start = -1;
        ...
        for (long i = 0; i < total_frames; i += window_frames) {
            ...
            double sum_sq = 0.0;
            float w_peak = 0.0f;
            long frame_count = w_end - i;

            for (long frame = i; frame < w_end; ++frame) {
                for (int c = 0; c < channels; ++c) {
                    float value = audio_buffer[frame * channels + c];
                    float abs_value = std::abs(value);
                    w_peak = std::max(w_peak, abs_value);
                    sum_sq += static_cast<double>(value) * static_cast<double>(value);
                }
            }

            double sample_count = static_cast<double>(frame_count) * static_cast<double>(channels);
            float w_rms = static_cast<float>(std::sqrt(sum_sq / std::max(1.0, sample_count)));

            bool thickness_match =
                (w_rms <= rms_threshold) &&
                (w_peak <= peak_threshold);
            ...
            if (thickness_match) {
                ...
                last_match_end = w_end;
                bridge_start = -1;
            } else if (current_match_start >= 0) {
                /*
                 * Çok kısa bir sapmayı hemen kapatmak yerine küçük bir
                 * köprü alanı olarak bekletiyoruz.
                 */
                if (bridge_start < 0) {
                    bridge_start = i;
                }

                long bridge_length = w_end - bridge_start;
                if (bridge_length > max_bridge_frames) {
                    long match_end = bridge_start;

                    if ((match_end - current_match_start) >= min_matching_frames) {
                        segments_to_delete.push_back({current_match_start, match_end});
                    }
        ...
        if (current_match_start >= 0 && last_match_end > current_match_start) {
            long match_end = last_match_end;
            if ((match_end - current_match_start) >= min_matching_frames) {
                segments_to_delete.push_back({current_match_start, match_end});
            }
        }
        ...
        /*
         * Kullanıcının seçtiği örnek, algoritmik tolerans dışında kalmış olsa
         * bile kesin olarak silinir. Böylece :: butonu seçili alanı hiçbir
         * durumda geride bırakmaz.
         */
        segments_to_delete.push_back({start_frame, end_frame});

        /*
         * Çakışan ve birbirine çok yakın segmentleri tek parçaya indir.
         * Böylece erase işlemi daha güvenli ve deterministik olur.
         */
        std::sort(segments_to_delete.begin(), segments_to_delete.end(),
        ...
        if (!segments_to_delete.empty()) {
            ...
            for (size_t i = 1; i < segments_to_delete.size(); ++i) {
                ...
                /*
                 * Küçük bir aralıkla ayrılmış iki aynı kalınlık bölgesini de
                 * tek silme alanına birleştiriyoruz. Bu, eski algoritmadaki
                 * "küçük delik bırakma" davranışını ortadan kaldırır.
                 */
                if (segments_to_delete[i].first <= last.second + max_bridge_frames) {
                    last.second = std::max(last.second, segments_to_delete[i].second);
                } else {
                    merged_segments.push_back(segments_to_delete[i]);
                }
        ...
        /*
         * Undo snapshot yalnızca gerçekten uygulanacak silme bulunduğunda
         * alınır. Böylece başarısız/boş taramalar gereksiz history adımı
         * üretmez.
         */
        push_history();

        size_t deleted_frames = 0;
        for (auto it = merged_segments.rbegin(); it != merged_segments.rend(); ++it) {
            long s_frame = std::max(0L, std::min(it->first, total_frames));
            long e_frame = std::max(0L, std::min(it->second, total_frames));

            if (s_frame >= e_frame) {
                continue;
            }

            long s_idx = s_frame * channels;
            long e_idx = e_frame * channels;

            if (s_idx >= 0 &&
                e_idx >= s_idx &&
                e_idx <= static_cast<long>(audio_buffer.size())) {
                deleted_frames += static_cast<size_t>(e_frame - s_frame);
                audio_buffer.erase(audio_buffer.begin() + s_idx,
                                   audio_buffer.begin() + e_idx);
            }
        }

        recalculate_envelope_data();
        log_edl_command(
            ActionType::DELETE_BY_THICKNESS,
            "DELETE_THICKNESS",
            start_ms,
            end_ms,
            "Deep full-track thickness scan deleted " +
            std::to_string(merged_segments.size()) +
            " matching/quiet regions (" +
            std::to_string(deleted_frames) +
            " frames)."
        );

        std::cout << "Thickness based deep deletion completed. "
                  << "Segments deleted: " << merged_segments.size()
                  << ", frames deleted: " << deleted_frames << std::endl;
        return 0;
    }
    ...
    int save_to_file(const std::string& filePath) {
        ...
        sf_count_t frames_to_write = static_cast<sf_count_t>(
            audio_buffer.size() / static_cast<size_t>(channels)
        );
        sf_count_t frames_written = sf_writef_float(
            outfile, audio_buffer.data(), frames_to_write
        );
        int close_result = sf_close(outfile);

        if (frames_written != frames_to_write) {
            std::cerr << "Error writing output file: " << filePath
                      << " - " << sf_strerror(nullptr) << std::endl;
            return -1;
        }

        if (close_result != 0) {
            std::cerr << "Error closing output file: " << filePath
                      << " - " << sf_strerror(nullptr) << std::endl;
            return -1;
        }
