...
import numpy as np
import zipfile  # .sound paketleme sistemi için eklendi
...
class WaveformWidget(QWidget):
    ...
    def set_width_scale(self, scale: int):
        ...
        # Sabit 300 saniye (300000 ms) baz genişlik hesaplamasını seçilen ölçeğe göre daraltarak
        # dalganın ekrandaki yayılımını (genişliğini) kusursuz oranla arttırırız.
...
class TemporaryTextPanel(QFrame):
    def __init__(self, parent=None):
        ...
        self.setFixedHeight(120)  # Varsayılan yükseklik
...
class SoundEditorWindow(QWidget):
    ...
    def __init__(self, core_window_ref=None):
        ...
        self._engine_cleaned = False # Temizlik bayrağı

        # Geçici metin boyutunu tutacak değişken
        self.current_text_font_size = 14
        self.waveform_scale = 1

        # Tüm ayarları tek bir seferde yükle (kalıcı)
        self._load_settings()
        ...
        if self.lib:
            ...
            if not self.audio_engine:
                ...
                # Varsayılan mikrofon efektlerini ayarla (UI'dan değiştirilemez)
        ...
        self.filter_overlay = None   # Overlay referansı (kullanılmıyor, artık gösterilmeyecek)

        # Geçici alt panel bileşeni oluşturuluyor
        ...
        # Oynatma durdurma güvenlik ağı
        self.destroyed.connect(self._cleanup_engine)
        
        # UI başladıktan sonra düğme durumlarını bir defa güncelle
        self.update_undo_redo_buttons()

    # --- Kavram.py ve dış etkenler için uyumluluk property'si ---
    @property
    def recording(self):
        """Kavram.py'nin editör kayıt durumunu kontrol edebilmesi için eklendi."""
        return getattr(self, 'is_recording_mode', False)

    # ----- GÜVENLİ OYNATMA VE MOTOR TEMİZLİĞİ -----
    def _cleanup_engine(self):
        """Uygulama/pencere kapandığında C++ motorunu tamamen durdurup yok eder."""
    ...
    def hideEvent(self, event):
        """Editör gizlendiğinde (başka bir editöre geçildiğinde vb.) arka planda oynamayı kes."""
        if self.audio_engine and self.lib:
            try:
                if self.lib.get_is_playing(self.audio_engine):
    ...
    def _load_settings(self):
        """Tüm arayüz ve filtre ayarlarını settings.json dosyasından yükler."""
    ...
    def _save_settings(self):
        """Tüm arayüz, filtre, boyut ve hizalama ayarlarını settings.json dosyasına kaydeder."""
        try:
            os.makedirs(SETTINGS_DIR, exist_ok=True)
            data = {
    ...
    def _set_default_microphone_effects(self):
        """C++ engine'e varsayılan efekt değerlerini gönderir."""
        if not self.lib or not self.audio_engine:
            return
        self.lib.set_mic_noise_gate_threshold(self.audio_engine, ctypes.c_float(0.000316))  # -70dB
        ...
        self.lib.set_mic_input_gain(self.audio_engine, ctypes.c_float(1.995))  # +6dB
    ...
    def init_ui(self):
        ...
        # --- YENİ UNDO/REDO BUTONLARI ---
        self.btn_undo = QPushButton()
        self.btn_undo.setIcon(create_svg_icon(SVG_UNDO_ICON, size=20))
        self.btn_undo.setStyleSheet(self.buttonStyleMini())
        self.btn_undo.setFixedSize(30, 30)
        self.btn_undo.setToolTip("Geri Al (Ctrl+Z)")
        self.btn_undo.clicked.connect(self.undo_action)

        self.btn_redo = QPushButton()
        self.btn_redo.setIcon(create_svg_icon(SVG_REDO_ICON, size=20))
        self.btn_redo.setStyleSheet(self.buttonStyleMini())
        self.btn_redo.setFixedSize(30, 30)
        self.btn_redo.setToolTip("İleri Al (Ctrl+Shift+Z)")
        self.btn_redo.clicked.connect(self.redo_action)
        # -------------------------------

        self.btn_cut = QPushButton('Cut')

        # --- YENİ BENZER SESLERİ OTOMATİK SİLME BUTONU (::) ---
        self.btn_similar_delete = QPushButton('::')
        self.btn_similar_delete.setToolTip("Sol Tık: Seçili kesite frekans olarak benzerleri siler\nSağ Tık: Seçili alanın kalınlığındaki (volume/sessizlik) tüm boşlukları güvenlice siler")
        self.btn_similar_delete.setFixedSize(30, 30)
        self.btn_similar_delete.setStyleSheet(self.buttonStyle())
        self.btn_similar_delete.clicked.connect(self.delete_similar_segments_action)
        
        # Yeni Eklenen: Sağ tıklama menüsü policy'si ve bağlantısı
        self.btn_similar_delete.setContextMenuPolicy(Qt.CustomContextMenu)
        self.btn_similar_delete.customContextMenuRequested.connect(self.delete_thickness_segments_action)
        # -----------------------------------------------------
        ...
        self.btn_export.customContextMenuRequested.connect(lambda pos: self.export_sound_package())

        self.btn_filter = QPushButton('I')
        self.btn_filter.setToolTip("Aktif filtreleri uygula (I butonu) - kalıcıdır")
        self.btn_filter.setStyleSheet(self.buttonStylePressure(self.noise_filter_enabled))
        self.btn_filter.setFixedSize(30, 30)
        self.btn_filter.clicked.connect(self.toggle_noise_filter)
        self.btn_filter.setContextMenuPolicy(Qt.NoContextMenu) 

        # --- DALGA FORMU GENİŞLİK AYARI BUTONU (YENİ GÜNCELLEME) ---
        self.btn_wave_scale = QPushButton(str(self.waveform_scale))
        self.btn_wave_scale.setStyleSheet(self.buttonStylePressure(False)) # Tasarımı 'I' (filtre) butonu ile aynı
        self.btn_wave_scale.setFixedSize(30, 30)
        self.btn_wave_scale.setToolTip("Dalga Formu Genişliği (1-7)")
        self.btn_wave_scale.clicked.connect(self.show_wave_scale_menu)
        # ------------------------------------------------------------
        ...
        self.speed_box.setFixedSize(100, 30)
        ...
        self.btn_panel_toggle = QPushButton('/')
        self.btn_panel_toggle.setToolTip("Geçici Metin Panelini Aç/Kapat")
        self.btn_panel_toggle.setStyleSheet(self.buttonStylePressure(self.temporary_text_panel.isVisible()))
        self.btn_panel_toggle.setFixedSize(30, 30)
        self.btn_panel_toggle.clicked.connect(self.toggle_text_panel)

        self.btn_center_text = QPushButton('O')
        self.btn_center_text.setToolTip("Metni Ortala / Sola Yasla")
        self.btn_center_text.setStyleSheet(self.buttonStylePressure(self.text_alignment_center))
        self.btn_center_text.setFixedSize(30, 30)
        self.btn_center_text.clicked.connect(self.toggle_text_alignment)

        self.btn_text_font_size = QPushButton(str(self.current_text_font_size))
        self.btn_text_font_size.setToolTip("Geçici Metin Boyutunu Ayarla")
        self.btn_text_font_size.setStyleSheet(self.buttonStyle())
        self.btn_text_font_size.setFixedSize(55, 30) 
        self.btn_text_font_size.clicked.connect(self.show_text_font_size_menu)
        self.btn_text_font_size.wheelEvent = self.change_text_font_size_by_wheel
        ...
        bar_layout.addWidget(self.quick_save_button)
        bar_layout.addWidget(self.btn_undo)
        bar_layout.addWidget(self.btn_redo)
        bar_layout.addWidget(self.btn_cut)
        bar_layout.addWidget(self.btn_similar_delete) # Yeni otomatik benzer temizleyici buton Cut'ın sağında
        bar_layout.addWidget(self.btn_del)
        bar_layout.addWidget(self.btn_play)
        bar_layout.addWidget(self.btn_enter)
        bar_layout.addWidget(self.btn_filter)
        bar_layout.addWidget(self.btn_wave_scale)     # I butonunun yanına ölçek menüsü
        bar_layout.addWidget(self.speed_box)
        bar_layout.addWidget(self.scroll_step_box)
        bar_layout.addWidget(self.btn_panel_toggle)
        bar_layout.addWidget(self.btn_center_text)
        bar_layout.addWidget(self.btn_text_font_size)
    ...
    # --- Dalga Formu Genişlik Menüsü (YENİ GÜNCELLEME) ---
    def show_wave_scale_menu(self):
        """Dalga formunun yatay genişliğini ayarlayan menüyü açar."""
        # Oynatma veya kayıt sırasında değiştirilmesini engelle (hata vermez, sessizce yok sayar)
        ...
        for scale in range(1, 8): # 1'den 7'ye kadar seçenekler eklendi
            action_text = f"{scale}"
    ...
    def toggle_text_panel(self):
        """Geçici metin panelinin gösterilme durumunu değiştirir."""
    ...
    def toggle_text_alignment(self):
        """Metin ortalama durumunu değiştirir ve tüm içeriği günceller."""
    ...
    def apply_text_alignment(self):
        """Geçici metin kutusundaki tüm paragrafları seçilen hizalamaya ve artırılmış satır aralığına göre günceller."""
    ...
    def on_text_changed(self):
        """Yazma veya kopyala-yapıştır yapıldığında tüm yeni satırların hizalanmasını ve boşluğunu korur."""
        text_edit = self.temporary_text_panel.text_edit
    ...
    def show_text_font_size_menu(self):
        """Geçici metin editörünün yazı boyutu menüsünü açar."""
        menu = QMenu(self)
        menu.setStyleSheet(self.menuStyle())
    ...
    def set_text_font_size(self, size):
        """Geçici metin editörünün yazı boyutunu günceller."""
    ...
    def change_text_font_size_by_wheel(self, event):
        """Yazı boyutu butonu üzerinde fare tekerleği çevrildiğinde yazı boyutunu değiştirir."""
    ...
    def load_wav_files_into_engine(self, file_paths):
        ...
        if file_paths:
            ...
            if result == 0:
                ...
                self.update_waveform()
                self.update_ui()
                self.update_undo_redo_buttons() # Butonları güncelle
                self.btn_play.setText('Play')
                self.waveform_widget.clear_selection()
    ...
    def delete_selected_segments(self):
        ...
        # Sildikten sonra buton durumlarını güncelle
        self.update_undo_redo_buttons()
    ...
    # --- BENZER SESLERİ OTOMATİK SİLME AKSİYONU (PYTHON TARAKI / SOL TIK) ---
    def delete_similar_segments_action(self):
        """Sol tık: Seçili aralığı analiz edip ses genelindeki %80 frekans benzerlerini siler."""
        if not self.lib or not self.audio_engine:
            return
        ...
        # Çoklu seçim varsa en ilkini örnek şablon alalım
        ...
        # C++ Algoritmasını %80 (0.80) eşik değeriyle çağır
        ...
        if result == 0:
            ...
            self.update_undo_redo_buttons()
            self.waveform_widget.clear_selection()
            self.waveform_widget.split_points_ms = [] # Benzer silmede zaman haritası değiştiği için split pointleri sıfırlayalım
            
            # Zaman çizelgesini güvenli bir pozisyona al (0 ms varsayılan)
    ...
    # --- YENİ EKLENEN: KALINLIĞA (GENLİĞE/VOLUME) GÖRE BOŞLUKLARI SİLME (SAĞ TIK) ---
    def delete_thickness_segments_action(self, pos=None):
        """Sağ tıklama: Seçilen sessizliğin kalınlığına (amplitude) sahip tüm boşlukları bulur ve güvenlice siler."""
        if not self.lib or not self.audio_engine:
            return
        
        selected_segments = self.waveform_widget.get_selected_segments()
        if not selected_segments:
            QMessageBox.information(self, 'Bilgi', 'Lütfen sağ tıklamadan önce örnek olarak silmek istediğiniz kalınlıktaki (boşluk/sessizlik) kesiti farenin sol tuşuyla seçin.')
            return
        ...
        # C++ Kalınlık Algoritmasını Çağır
        ...
        if result == 0:
            print("Thickness based gaps successfully analyzed and securely deleted.")
            self.update_undo_redo_buttons()
            self.waveform_widget.clear_selection()
            self.waveform_widget.split_points_ms = []
            ...
            QMessageBox.critical(self, 'Hata', 'Kalınlığa dayalı boşluk silme işlemi sırasında motor hatası oluştu.')
