...
class CoreWindow(QWidget):
    ...
    def load_custom_editors(self):
        ...
        # Önce temel sıralamayı al, sonra custom'ları ekle
        ...
        self.save_custom_editors_json()  # Eski JSON'u güncelle (uyumluluk)
    ...
    def switchToEditor(self, editor_name, close_current=False):
        if editor_name in self.editor_map and self.editor_map[editor_name] == 'CUSTOM':
            ...
            if executable and os.path.exists(executable):
                try:
                    # Çalışma dizinini executable'ın bulunduğu dizin yap
                    cwd = os.path.dirname(executable)
                    env = os.environ.copy()
                    # lib/ klasörü varsa LD_LIBRARY_PATH'a ekle
