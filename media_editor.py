...
# AiEditor ile aynı tasarım Save İkonu (Renk uyumu için #eee kullanıldı)
...
# Artık her şey medya_cut içinde toplanıyor, _v&s_ kullanılmıyor.
...
# Önce lib/ altındaki dosyayı dene, yoksa kök dizindekini dene
_lib_candidate = resource_path("lib/libmediaengine.so")
if os.path.exists(_lib_candidate):
    LIB_PATH = _lib_candidate
else:
    LIB_PATH = resource_path("libmediaengine.so")
    print(f"Uyarı: {_lib_candidate} bulunamadı, {LIB_PATH} deneniyor.")
...
class AdvancedFilterDialog(QDialog):
    ...
    def load_initial_settings(self):
        ...
        # Compressor Ratio
        ratio_map = {"1:1": 1, "2:1": 2, "3:1": 3, "4:1": 4, "5:1": 5, "8:1": 10, "10:1": 10}
...
class MediaEditor(QWidget):
    ...
    def __init__(self, core_window_ref=None, parent=None):
        ...
        # --- YENİ: Kayıtlı proje yolu takibi ---
        ...
        # Bütün kayıtlar medya_cut içinde toplansın
        self.audio_record_dir = SAVE_DIR
        self._ensure_dirs_exist()
        self._clean_save_dir_on_startup() # Her yeni açılışta içeriği tamamen yok et
    ...
    def _clean_save_dir_on_startup(self):
        """Program ilk açıldığında SAVE_DIR içeriğini tamamen ve doğrudan yok eder."""
        try:
            if os.path.exists(self.media_save_dir):
                for filename in os.listdir(self.media_save_dir):
                    file_path = os.path.join(self.media_save_dir, filename)
                    try:
                        ...
                        print(f"Startup: Dosya silinemedi: {file_path}. Hata: {e}")
                print(f"Startup: {self.media_save_dir} kalıcı olarak temizlendi.")
        except Exception as e:
            print(f"Startup temizleme hatası: {e}")

    # YENİ: Filtreleme ile ilgili metodlar (camera_editor.py'den kopyalandı)
    def load_filter_settings(self):
        """Filtre ayarlarını JSON dosyasından yükler (noise_filter_enabled dahil)."""
        try:
    ...
    def save_filter_settings(self):
        ...
        try:
            self.filter_settings['noise_filter_enabled'] = self.noise_filter_enabled
    ...
    def _load_noise_filter_setting(self):
        """Kaydedilmiş I butonu durumunu overlay_settings.json'dan (ortak) yükler."""
        try:
            ...
            print(f"Python: I butonu ayarı yüklenemedi: {e}")
        return False

    def _save_noise_filter_setting(self):
        """I butonu durumunu overlay_settings.json'a (ortak) kaydeder."""
        try:
            data = {}
            if os.path.exists(OVERLAY_SETTINGS_FILE):
                with open(OVERLAY_SETTINGS_FILE, 'r') as f:
            ...
            print(f"Python: I butonu ayarı kaydedilemedi: {e}")
    ...
    def toggleNoiseFilter(self):
        """Gürültü filtresi durumunu değiştirir, buton stilini günceller ve kalıcı olarak kaydeder."""
        if AUDIO_LIBS_MISSING:
            show_missing_libs_error(self)
        ...
        # Durumu ortak filter settings dosyasına (overlay) kaydet
        self._save_noise_filter_setting()
        # Geriye dönük uyumluluk için kendi ayarına da kaydet
        self.save_filter_settings()
        
        print(f"Python: Gürültü filtresi {'etkin' if self.noise_filter_enabled else 'devre dışı'} (kalıcı).")

        # Kavram.py'ye baglanti durumunu bildir
        if self.core_window_ref:
            self.core_window_ref.set_media_filter_connection(self.noise_filter_enabled)
    ...
    def init_ui(self):
        ...
        # YENİ EKLENEN HIZLI KAYDET BUTONU
        self.save_button = QPushButton()
        # İkon rengini beyaz (white) olarak ayarlıyoruz (File butonu metni ile uyumlu)
        self.save_button.setIcon(create_svg_icon(SVG_SAVE_ICON, size=20, color="white"))
        self.save_button.setStyleSheet(self.buttonStyleMini())
        self.save_button.setFixedSize(30, 30)
        self.save_button.setToolTip("Quick Save (Hızlı Kaydet)")
        # --- GÜNCELLEME: Butonun fonksiyonu değiştirildi ---
        # Artık export_media_archive yerine quick_save_media çağrılıyor
        self.save_button.clicked.connect(self.quick_save_media)
        # --- GÜNCELLEME SONU ---
        ...
        self.play_pause_btn.setFixedSize(88, 30)
        ...
        self.cut_btn.setFixedSize(60, 30)
        ...
        self.noise_filter_button.setToolTip("Noise Reduction Filter (Click to On/Off) - Kalıcıdır")
        self.noise_filter_button.setStyleSheet(self.buttonStylePressure(self.noise_filter_enabled))
        ...
        # YENİ EKLENEN S BUTONU (Klasörü Temizle ve Sıfırla)
        self.clear_folders_btn = QPushButton("S")
        self.clear_folders_btn.setToolTip("Klasörü Temizle (medya_cut) ve Sıfırla")
        self.clear_folders_btn.setStyleSheet(self.buttonStyleMini())
        self.clear_folders_btn.setFixedSize(30, 30)
        self.clear_folders_btn.clicked.connect(self.clear_media_folders)
        top_buttons_layout.addWidget(self.clear_folders_btn)
        ...
        # Export Button (Modified for Direct Right Click)
        self.export_btn = QPushButton("Export")
        # Sol tık (.media export)
        self.export_btn.clicked.connect(self.export_media_archive)
        # Sağ tık (.mkv export) - Direkt fonksiyon çağrısı
        self.export_btn.setContextMenuPolicy(Qt.CustomContextMenu)  
        self.export_btn.customContextMenuRequested.connect(self.export_timeline_to_mkv) 
    ...
    def clear_media_folders(self):
        """medya_cut klasörünün içini temizler ve programı sıfırlar."""
        reply = QMessageBox.question(self, 'Temizleme Onayı',
                                     "medya_cut klasörünün içindeki tüm dosyaları silmek ve zaman çizelgesini tamamen sıfırlamak istediğinize emin misiniz?",
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                # 2. Programı (Arayüz ve Zaman Çizelgesini) Sıfırla
                self.stop_media()
                ...
                self.current_time_label.setText(self.format_time_display_with_ms(0))
                self.update_total_duration_label()
                self._update_sequence_play_button_state()
                
                # Fiziksel dosyalar silindiği için Undo/Redo geçmişini de sıfırla ki Ctrl+Z ile çökme olmasın
                self._undo_stack.clear()
                self._redo_stack.clear()
                self._save_state()
                self._update_undo_redo_buttons()
                
                print("Klasör temizlendi ve editör başarıyla sıfırlandı.")
            except Exception as e:
                print(f"Klasör temizleme hatası: {e}")
                self.show_message_box("Hata", f"Temizleme sırasında hata: {e}")

    def closeEvent(self, event):
        # --- GÜVENLİK PROTOKOLÜ BAŞLANGICI ---
        if getattr(self, 'is_recording', False):
            ...
            reply = msg.exec_()
            if reply == QMessageBox.Yes:
                print("Python: Kullanıcı kapatmayı onayladı → kayıt iptal ediliyor.")
                self._stop_all_recording_processes()
                self.is_recording = False
                if hasattr(self, 'audio_mode_btn'):
                    self.audio_mode_btn.setText("Sound")
                self._update_record_button_state()
            else:
                event.ignore()
                return
        # --- GÜVENLİK PROTOKOLÜ SONU ---
    ...
    def triggerCoreSwitcher(self):
        ...
        if getattr(self, 'is_recording', False):
            ...
            reply = msg.exec_()
            if reply == QMessageBox.Yes:
                print("Python: Kullanıcı editör geçişini onayladı → kayıt iptal ediliyor.")
                self._stop_all_recording_processes()
                self.is_recording = False
                self.is_audio_mode_selected = False
                self.is_camera_mode_selected = False
        ...
        try:
            ...
            if self.current_media_index == -1 or not self.media_queue:
                ...
                if self.position < EPSILON_POSITION:
                    ...
                    if self.position < self.MIN_SEGMENT_DURATION_SEC or \
                       (current_segment['duration'] - self.position) < self.MIN_SEGMENT_DURATION_SEC:
                        # Kesme yapma, yeni segmenti mevcut segmentin ÖNÜNE ekle ("bir üste")
                        self.media_queue.insert(self.current_media_index, new_segment_data)
                        self.current_media_index += 1
                        self.position = 0.0
                        self.paused_position = 0.0
                        self.show_message_box("Info", "Ses dosyası, kesme işlemi yapılmadan mevcut segmentin önüne eklendi (1.1 saniye kuralı).")
                    else:
                        # Normal kesme işlemini dene
                        ...
                        part1_after_cut, part2_after_cut = self._perform_cut_at_position(
                            original_segment_index_before_cut, cut_point_for_active_segment
                        )
                        if part1_after_cut or part2_after_cut:
                            ...
                            # Kesme başarısız (örneğin C++ hatası) → yine öne ekle
                            self.media_queue.insert(self.current_media_index, new_segment_data)
                            self.current_media_index += 1
                            self.position = 0.0
                            self.paused_position = 0.0
                            self.show_message_box("Warning", "Otomatik kesme işlemi başarısız oldu. Yeni segment mevcut segmentin önüne eklendi.")
                    # ---------------------------------------------
    ...
    # ----- YENİ: Filtreleme ile dosya ekleme için yardımcı metodlar -----
    def _extract_audio_from_video(self, video_path, output_wav_path):
        """FFmpeg kullanarak videodan sesi WAV olarak çıkarır."""
        cmd = [
            FFMPEG_PATH, "-i", video_path,
            "-vn",                 # Video stream'i atla
            "-acodec", "pcm_s16le", # PCM 16-bit little-endian
            "-ar", "48000",        # 48 kHz örnekleme
            "-ac", "1",            # Mono
            "-y",                  # Üzerine yaz
            output_wav_path
        ]
        try:
            ...
            print(f"FFmpeg extraction error: {e.stderr}")
            return False

    def _replace_audio_in_video(self, video_path, audio_wav_path, output_video_path):
        """FFmpeg kullanarak videodaki sesi filtrelenmiş ses ile değiştirir."""
        cmd = [
            FFMPEG_PATH, "-i", video_path,
            "-i", audio_wav_path,
            "-c:v", "copy",        # Video codec'ini kopyala (hızlı)
            "-c:a", "aac",         # Ses codec'i AAC
            "-b:a", "192k",        # Bitrate
            "-map", "0:v:0",       # Videoyu ilk girişten al
            "-map", "1:a:0",       # Sesi ikinci girişten al
            "-shortest",           # En kısa süreye göre kes
            "-y",                  # Üzerine yaz
            output_video_path
        ]
        try:
            ...
            print(f"FFmpeg remux error: {e.stderr}")
            return False

    def _apply_filter_to_media(self, original_path, media_type, original_name, callback):
        """
        Filtre aktifse medya dosyasını filtreler ve sonucu callback ile döndürür.
        callback(filtered_path, media_type, original_name) şeklinde çağrılır.
        """
        if not self.noise_filter_enabled or media_type == 'media_archive':
            # Filtre kapalı veya arşiv: direkt orijinali kullan
            if callback:
                callback(original_path, media_type, original_name)
            return

        # Geçici dizin oluştur
        temp_dir = tempfile.mkdtemp(prefix="media_filter_")
        ...
        if media_type == 'audio':
            # Ses dosyasını filtrele
            def on_audio_filtered(success, output_path, message):
                if success and output_path and os.path.exists(output_path):
                    callback(output_path, media_type, original_name)
                else:
                    print(f"Audio filtering failed: {message}")
                    callback(original_path, media_type, original_name)
                cleanup_temp()
            
            self.core_window_ref.process_audio_with_filter(original_path, on_audio_filtered)
        
        elif media_type == 'video':
            # Video: sesi çıkar, filtrele, tekrar birleştir
            extracted_audio = os.path.join(temp_dir, "extracted_audio.wav")
            if not self._extract_audio_from_video(original_path, extracted_audio):
                print("Failed to extract audio from video")
                callback(original_path, media_type, original_name)
                cleanup_temp()
                return
            
            def on_video_audio_filtered(success, filtered_audio_path, message):
                if success and filtered_audio_path and os.path.exists(filtered_audio_path):
                    # Filtrelenmiş sesi video ile birleştir
                    ...
                    while os.path.exists(output_video):
                        ...
                        counter += 1
                    if self._replace_audio_in_video(original_path, filtered_audio_path, output_video):
                        callback(output_video, media_type, original_name)
                    else:
                        print("Failed to replace audio in video")
                        callback(original_path, media_type, original_name)
                else:
                    print(f"Video audio filtering failed: {message}")
                    callback(original_path, media_type, original_name)
                cleanup_temp()
            
            self.core_window_ref.process_audio_with_filter(extracted_audio, on_video_audio_filtered)
        else:
            callback(original_path, media_type, original_name)
            cleanup_temp()

    # --- select_media_files ve load_file güncellemeleri ---
    def select_media_files(self):
        ...
        progress_dialog = QProgressDialog("Importing and filtering media...", "Cancel", 0, total_files, self)
        ...
        self._import_queue = list(file_paths)
        self._import_index = 0
        self._import_progress = progress_dialog
    ...
    def _import_next(self):
        ...
        original_path = self._import_queue[self._import_index]
        self._import_progress.setLabelText(f"Processing {os.path.basename(original_path)}...")
        QApplication.processEvents()
        ...
        if media_type == 'media_archive':
            ...
            self.update_total_duration_label()
            self._save_state()
            if self._process_media_archive(original_path):
                pass  # arşiv başarıyla yüklendi
            self._import_index += 1
            self._import_progress.setValue(self._import_index)
            QTimer.singleShot(10, self._import_next)
        elif media_type != 'unknown':
            # Filtre uygula
            def on_filtered(file_path, mtype, orig_name):
                self.add_media_to_queue(file_path, mtype, orig_name)
                self._import_index += 1
                self._import_progress.setValue(self._import_index)
                QTimer.singleShot(10, self._import_next)
            self._apply_filter_to_media(original_path, media_type, original_filename_no_ext, on_filtered)
        else:
            self.show_message_box("Warning", f"Bilinmeyen dosya türü atlandı: {os.path.basename(original_path)}")
            self._import_index += 1
            self._import_progress.setValue(self._import_index)
            QTimer.singleShot(10, self._import_next)
    ...
    def load_file(self, file_path: str):
        ...
        if media_type == 'media_archive':
            ...
            self.update_total_duration_label()
            self._save_state()
            if not self._process_media_archive(file_path):
                self.show_message_box("Error", f"Medya arşivi '{os.path.basename(file_path)}' yüklenemedi.")
        elif media_type != 'unknown':
            # Filtre uygula ve ekle
            def on_filtered(filtered_path, mtype, orig_name):
                self.add_media_to_queue(filtered_path, mtype, orig_name)
                # Yükleme sonrası güncellemeler
                if self.media_queue and self.current_media_index == -1:
                    self.current_media_index = 0
                    self.position = 0
                    self.paused_position = 0
                ...
                self._update_sequence_play_button_state()
                self._save_state()
            self._apply_filter_to_media(file_path, media_type, original_filename_no_ext, on_filtered)
        else:
            self.show_message_box("Warning", f"Bilinmeyen dosya türü veya geçersiz medya: {os.path.basename(file_path)}")
    # ----------------------------------------------------------------
    ...
    def _process_media_archive(self, archive_path: str) -> bool:
        ...
        try:
            ...
            # --- YENİ: Başarıyla yüklenen projenin yolunu hafızaya al ---
            self.current_project_path = archive_path
            print(f"Project loaded, path remembered: {self.current_project_path}")
            # -----------------------------------------------------------
    ...
    def cut_media(self):
        if not self.media_queue:
            self.show_message_box("Warning", "Kesmek için lütfen önce medya dosyalarını seçin!")
            return
        if self.current_media_index == -1:
            self.show_message_box("Warning", "Kesme işlemi için aktif segment seçilmedi.")
            return
        if not self.media_engine:
            self.show_message_box("Error", "Medya motoru (libmediaengine.so) yüklenemedi. Kesme işlemi gerçekleştirilemez.")
            return
        if self.is_playing:
            self.show_message_box("Warning", "Kesme işlemi için medya oynatmayı duraklatın!")
            return
        ...
        # ---------- EKLENEN GÜVENLİK KONTROLÜ ----------
        if relative_cut_point_sec < self.MIN_SEGMENT_DURATION_SEC or \
           (original_segment['duration'] - relative_cut_point_sec) < self.MIN_SEGMENT_DURATION_SEC:
            self.show_message_box("Warning", "Kesme işlemi iptal edildi: Oluşacak parçalardan biri 1.1 saniyeden kısa.")
            return
        # ---------------------------------------------

        EPSILON_CUT = 10
        if abs(relative_cut_point_ms) < EPSILON_CUT:
            relative_cut_point_ms = 0
        elif abs(relative_cut_point_ms - original_duration_ms) < EPSILON_CUT:
            relative_cut_point_ms = original_duration_ms

        if relative_cut_point_ms == 0 or relative_cut_point_ms == original_duration_ms:
            return
        ...
        for s in self.media_queue:
            ...
            if match:
                existing_sub_numbers_for_base.append(int(match.group(1)))
        ...
        cut_result = self.media_engine.cut_media_segment(
            original_file_path.encode('utf-8'),
            relative_cut_point_ms,
            output_file_1_path.encode('utf-8'),
            output_file_2_path.encode('utf-8'),
            original_segment['type'].encode('utf-8')
        )
        if cut_result != 0:
            self.show_message_box("Error", f"C++ motoru kullanılarak medya kesilemedi. Hata kodu: {cut_result}")
            return

        try:
            os.remove(original_file_path)
        except OSError as e:
            print(f"Error: An error occurred while deleting the original temporary file: {original_file_path} - {e}")
        ...
        if first_part_actual_duration_ms == -1 or second_part_actual_duration_ms == -1:
            ...
            if os.path.exists(output_file_1_path):
                os.remove(output_file_1_path)
            if os.path.exists(output_file_2_path):
                os.remove(output_file_2_path)
            if not self.media_queue:
                self.current_media_index = -1
                self.position = 0
                self.paused_position = 0
        ...
        if add_segment_2:
            new_segment_2 = {
                'path': output_file_2_path,
                'type': original_segment['type'],
                'duration': second_part_duration_sec,
                'abbreviation': new_abbreviation_2,
                'original_name_prefix': original_segment['original_name_prefix'],
                'formatted_duration': self.format_time_display(second_part_duration_sec),
                'formatted_duration_ms': self.format_time_display_with_ms(second_part_duration_sec),
                'original_ext': original_source_ext,
                'id': str(time.time()) + new_abbreviation_2,
                'is_cut_point': False
            }
        ...
        if add_segment_1:
            new_segment_1 = {
                'path': output_file_1_path,
                'type': original_segment['type'],
                'duration': first_part_duration_sec,
                'abbreviation': new_abbreviation_1,
                'original_name_prefix': original_segment['original_name_prefix'],
                'formatted_duration': self.format_time_display(first_part_duration_sec),
                'formatted_duration_ms': self.format_time_display_with_ms(first_part_duration_sec),
                'original_ext': original_source_ext,
                'id': str(time.time()) + new_abbreviation_1,
                'is_cut_point': True
            }
    ...
    def _advance_to_next_segment(self):
        ...
        if next_index < len(self.media_queue):
            ...
            # ----- EKLENEN PLAYER RESET (Yeşil kare ve oran bozulmasını engellemek için) -----
            self.player.stop()
            self.player.setMedia(QMediaContent())
            QApplication.processEvents()
            # --------------------------------------------------------------------------------
    ...
    # --- YENİ EKLENEN: Hızlı Kaydet Fonksiyonu ---
    def quick_save_media(self):
        """
        Eğer bir proje daha önce kaydedildiyse veya yüklendiyse,
        o dosyanın üzerine yazar (Quick Save).
        Aksi takdirde Farklı Kaydet (Export) penceresini açar.
        """
        if not self.media_queue:
            self.show_message_box("Warning", "Kaydedilecek medya yok."); 
            return

        if self.current_project_path:
            # Daha önce bir yol belirlenmiş, doğrudan oraya kaydet
            self._save_project_internal(self.current_project_path)
        else:
            # Yol yok, Export (Farklı Kaydet) fonksiyonunu çağır
            self.export_media_archive()
    # ----------------------------------------------

    # --- GÜNCELLENEN: Export Fonksiyonu (Refactored) ---
    def export_media_archive(self):
        """Sol tık: .media (Proje) olarak dışa aktarır."""
        ...
        # Otomatik isim yerine boş isim veya sadece klasör
        output_file_user_selected, _ = QFileDialog.getSaveFileName(self, "Medya Arşivini Dışa Aktar", DEFAULT_BASE_DIR, "Medya Arşivi (*.media)")
        ...
        # Asıl kaydetme işlemini iç fonksiyona devret
        self._save_project_internal(output_file_user_selected)

    def _save_project_internal(self, target_path):
        """
        Verilen target_path yoluna mevcut projeyi kaydeder.
        Bu fonksiyon hem Export hem de Quick Save tarafından kullanılır.
        """
        temp_export_dir = tempfile.mkdtemp(prefix="media_export_")
        manifest_file_name_prefix = os.path.splitext(os.path.basename(target_path))[0]
        ...
        progress_dialog.setWindowTitle("Saving Project")
        ...
        export_label = QLabel(f"Saving to {os.path.basename(target_path)}...")
        ...
        self.time_estimate_label = QLabel("Processing segments...")
        ...
        try:
            ...
            archive_result = self.media_engine.archive_segments(target_path.encode('utf-8'), temp_export_dir.encode('utf-8'), manifest_file_name.encode('utf-8'))
            self.progress_bar.setValue(100)
            self.time_estimate_label.setText("Save complete!")
            
            if archive_result == 0: 
                # --- YENİ: Başarıyla kaydedilen yolu hafızaya al ---
                self.current_project_path = target_path
                print(f"Project saved, path updated: {self.current_project_path}")
                # --------------------------------------------------
                self.show_message_box("Save Successful", f"Project saved to:\n{target_path}")
            else: 
                self.show_message_box("Error", f"Could not save project. Error code: {archive_result}")
                
        except Exception as e: 
            self.show_message_box("Error", f"An error occurred while saving: {e}")
        finally:
            if os.path.exists(temp_export_dir): shutil.rmtree(temp_export_dir)
            progress_dialog.close()
    # -----------------------------------------------------------

    def export_timeline_to_mkv(self):
        """
        Sağ tık: .mkv (Render Video) olarak dışa aktarır.
        """
        if not self.media_queue:
            self.show_message_box("Warning", "Render alınacak medya yok. Lütfen zaman çizelgesine dosya ekleyin.")
            return
        ...
        try:
            ...
            self.show_message_box("Error", f"FFmpeg bulunamadı: {FFMPEG_PATH}. Lütfen uygulama dizininde bin/ffmpeg olduğundan emin olun.")
            return

        # Dosya kaydetme diyaloğu - Otomatik isim yok
        output_file_path, _ = QFileDialog.getSaveFileName(self, "Videoyu Render Al", DEFAULT_BASE_DIR, "Video Files (*.mkv)")
        
        if not output_file_path:
            return
        if not output_file_path.lower().endswith(".mkv"):
            output_file_path += ".mkv"

        # İşlem Diyaloğu
        progress = QProgressDialog("Rendering Video...", "Cancel", 0, len(self.media_queue) + 5, self)
        progress.setWindowModality(Qt.WindowModal)
        progress.setWindowTitle("Rendering")
        progress.show()
        QApplication.processEvents()

        temp_render_dir = tempfile.mkdtemp(prefix="render_mkv_")
        concat_list_path = os.path.join(temp_render_dir, "file_list.txt")

        try:
            # Görüntü Kaymasını Engelleme: Hedef çözünürlüğü ilk videodan al.
            target_width, target_height = 1920, 1080
            for seg in self.media_queue:
                if seg['type'] in ['video', 'recorded_video']:
            ...
            # 1. Adım: Ses dosyaları için dinamik boyutlu görsel placeholder oluştur
            placeholder_image_path = os.path.join(temp_render_dir, "audio_placeholder.png")
            self._create_audio_placeholder_image(placeholder_image_path, target_width, target_height)
            
            processed_files_for_concat = []

            # 2. Adım: Tüm segmentleri işle
            for i, segment in enumerate(self.media_queue):
                if progress.wasCanceled():
                    raise InterruptedError("User canceled operation")
                
                progress.setLabelText(f"Processing segment {i+1}/{len(self.media_queue)}: {segment['abbreviation']}")
                progress.setValue(i)
                QApplication.processEvents()

                original_path = segment['path']
                # Zaman damgası ve 124 Dk bug'ını çözmek için geçici mkv konteyneri.
                temp_mkv_path = os.path.join(temp_render_dir, f"segment_{i}.mkv")
                
                if segment['type'] in ['audio', 'recorded_audio']:
                    # ----- DEĞİŞİKLİK 3: _convert_audio_to_video_segment çağrısı target_width/height eklendi -----
                    success = self._convert_audio_to_video_segment(
                        original_path,
                    ...
                    if success:
                        processed_files_for_concat.append(temp_mkv_path)
                    else:
                        print(f"Warning: Failed to convert audio segment {i}")
                else:
                    # ----- DEĞİŞİKLİK 2: Video segmentleri de re-encode ediliyor (artık -c copy yok) -----
                    convert_cmd = [
                    ...
                    if res.returncode == 0:
                        ...
                        # Hata durumunda orijinali kullan (bu nadir olur)
                        print(f"Warning: Could not re-encode segment {i}, using original.")
                        processed_files_for_concat.append(os.path.abspath(original_path))

            # 3. Adım: Concat listesi oluştur
            with open(concat_list_path, 'w', encoding='utf-8') as f:
                for file_path in processed_files_for_concat:
                    # FFmpeg concat formatı: file '/path/to/file'
                    safe_path = file_path.replace("'", "'\\''")
                    f.write(f"file '{safe_path}'\n")

            # 4. Adım: FFmpeg ile birleştir (Render)
            progress.setLabelText("Merging and Encoding (This may take time)...")
            progress.setValue(len(self.media_queue))
            QApplication.processEvents()

            # Ham dosyalar hazırlandı. Şimdi sadece yan yana dikme (copy) yapıyoruz.
            ffmpeg_cmd = [
                FFMPEG_PATH,
                "-f", "concat",
                "-safe", "0",
                "-i", concat_list_path,
                "-c", "copy",          # Render (yeniden kodlama) yapmadan doğrudan kopyala/birleştir
                "-y",                  # Üzerine yaz
                output_file_path
            ]
            ...
            progress.setValue(len(self.media_queue) + 5)
            
            if result.returncode == 0:
                self.show_message_box("Render Successful", f"Video başarıyla oluşturuldu:\n{output_file_path}")
            else:
                print(f"FFmpeg Error Output: {result.stderr}")
                self.show_message_box("Render Error", f"Render sırasında hata oluştu. FFmpeg loglarını konsoldan kontrol edin.")

        except InterruptedError:
            print("Render cancelled by user.")
        except Exception as e:
            traceback.print_exc()
            self.show_message_box("Render Error", f"Beklenmeyen bir hata oluştu: {str(e)}")
        finally:
            progress.close()
    ...
    def _create_audio_placeholder_image(self, output_path, width, height):
        """Ses dosyaları için dinamik boyutta video arka planı olacak görsel oluşturur."""
        ...
        font_size = max(50, int(height * 0.4))
        font = QFont("Arial", font_size)
        painter.setFont(font)
        painter.setPen(QColor("#AAAAAA")) # Gri renk
    ...
    # ----- DEĞİŞİKLİK 1: _convert_audio_to_video_segment (target_width/height ve scale/pad eklendi) -----
    def _convert_audio_to_video_segment(self, audio_path, image_path, output_video_path, duration, target_width, target_height):
        """
        FFmpeg kullanarak tek bir ses dosyasını ve görseli birleştirip mkv dosyası oluşturur.
        """
        cmd = [
            FFMPEG_PATH, "-y",
            "-loop", "1",
    ...
    def export_single_video(self):
        # Bu fonksiyon artık kullanılmıyor, yerine export_timeline_to_mkv kullanılıyor
        pass
    ...
    def _toggle_audio_mode_selection(self):
        ...
        if not self.is_recording:
            ...
            # 1. Stop the recording and merge segments
            try:
                ...
                if stop_result == 0:
                    ...
                    if merge_result == 0:
                        # 2. Decide which audio file to add (filtered or original)
                        # We'll create a closure to add the final file and clean up
                        def add_audio(file_path):
                            """Add the given audio file to the timeline and clean up the original if needed."""
                            if not os.path.exists(file_path):
                                self.show_message_box("Error", "Kaydedilen ve birleştirilen ses dosyası bulunamadı.")
                                return
                            original_filename_no_ext = os.path.splitext(os.path.basename(file_path))[0]
                            self.add_media_to_queue(file_path, 'recorded_audio', original_filename_no_ext)
                            
                            # Nihai dosya kopyalandığı için geçici recorded_audio_*.wav dosyasını hemen siliyoruz.
                            try:
                                if os.path.exists(file_path):
                                    os.remove(file_path)
                                    print(f"Removed temp recorded file: {file_path}")
                            except Exception as e:
                                print(f"Failed to remove temp recorded file: {e}")
                                
                            # Eğer filtrelenmiş ses eklendiyse, orijinal kopyayı da siliyoruz.
                            if file_path != self.audio_record_final_path and os.path.exists(self.audio_record_final_path):
                                try:
                                    os.remove(self.audio_record_final_path)
                                    print(f"Removed original audio file: {self.audio_record_final_path}")
                                except Exception as e:
                                    print(f"Failed to remove original audio: {e}")

                        # 3. Apply filter if enabled
                        if self.noise_filter_enabled and self.core_window_ref:
                            # Use external filter through core window
                            def on_filter_complete(success, output_path, message):
                                if success and output_path and os.path.exists(output_path):
                                    print(f"MediaEditor: Filtreleme tamamlandi - {output_path}")
                                    add_audio(output_path)
                                else:
                                    print(f"MediaEditor: Filtreleme basarisiz - {message}")
                                    # Fallback to original
                                    if os.path.exists(self.audio_record_final_path):
                                        add_audio(self.audio_record_final_path)
                                    else:
                                        self.show_message_box("Error", "Filtreleme başarısız ve orijinal dosya bulunamadı.")
                                # Reset recording state after callback
                                self.is_recording = False
                                self.is_audio_mode_selected = False
                                self.audio_record_final_path = None
                                self.is_stopping_audio_recording = False
                            ...
                            filter_started = self.core_window_ref.process_audio_with_filter(
                                self.audio_record_final_path,
                                on_filter_complete
                            )
                            if not filter_started:
                                print("MediaEditor: Filtreleme baslatilamadi, orijinal dosya kullanilacak")
                                add_audio(self.audio_record_final_path)
                                ...
                                self.audio_mode_btn.setEnabled(True)
                                self._update_record_button_state()
                        elif self.noise_filter_enabled:
                            # Fallback: use internal Python filter
                            ...
                            # No filter: just add the original
                            add_audio(self.audio_record_final_path)
                            # Clean up state
                            self.is_recording = False
                            self.is_audio_mode_selected = False
